...
export class HttpRuntimeAdapter implements AgentRuntime {
  ...
  private async call<T>(method: string, urlPath: string, body?: unknown): Promise<T> {
    ...
    try {
      const res = await this.fetch(`${this.options.baseUrl}${urlPath}`, {
        method,
  ...
  async interrupt(session: AgentSession): Promise<void> {
    await this.call("POST", `/sessions/${session.sessionId}/interrupt`).catch(() => undefined);
  }

  async suspend(session: AgentSession): Promise<void> {
    await this.call("POST", `/sessions/${session.sessionId}/suspend`).catch(() => undefined);
    this.statuses.set(session.agentId, "SUSPENDED");
  }

  async resume(session: AgentSession): Promise<void> {
    await this.call("POST", `/sessions/${session.sessionId}/resume`).catch(() => undefined);
    this.statuses.set(session.agentId, "IDLE");
  }

  async stop(session: AgentSession): Promise<void> {
    await this.call("POST", `/sessions/${session.sessionId}/stop`).catch(() => undefined);
    this.statuses.set(session.agentId, "STOPPED");
  }

  async getStatus(session: AgentSession): Promise<AgentRuntimeStatus> {
    try {
      const r = await this.call<{ status: AgentRuntimeStatus }>("GET", `/sessions/${session.sessionId}`);
